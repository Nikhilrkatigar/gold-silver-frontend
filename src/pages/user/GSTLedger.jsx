...
export default function GSTLedger() {
    ...
    const fetchLedgers = async () => {
        try {
            ...
            console.log('🏦 GST Ledger Page - Ledgers fetched:', response?.data?.ledgers?.length || 0);
            console.log('📋 Ledger Details:', response?.data?.ledgers?.map(l => ({ name: l.name, type: l.ledgerType, hasVouchers: l.hasVouchers })));
