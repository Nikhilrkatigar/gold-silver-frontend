...
const ItemStockManagement = () => {
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      ...
      setEditingItem(null);
      setCategorySearch('');
      setFormData({ name: '', metal: 'gold', purity: '22K', grossWeight: '', lessWeight: '0', categoryId: '' });
      fetchData();
    } catch (error) {
  ...
  const handleExportCSV = () => {
    const headers = ['Item Code', 'Name', 'Metal', 'Purity', 'Gross Weight', 'Less Weight', 'Net Weight', 'Category', 'Status'];
  ...
  const stats = {
    ...
    totalGrossWeight: items.reduce((sum, i) => sum + (i.grossWeight || 0), 0),
    ...
    <PullToRefresh onRefresh={fetchData}>
      <div style={{ padding: '1rem' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginTop: '1rem' }}>
            <div className="card" style={{ padding: '1rem' }}>
              ...
              <div style={{ fontSize: '1.8rem', fontWeight: 'bold', marginTop: '0.5rem' }}>{stats.total}</div>
            </div>
            <div className="card" style={{ padding: '1rem' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Available</div>
              <div style={{ fontSize: '1.8rem', fontWeight: 'bold', marginTop: '0.5rem', color: '#10b981' }}>{stats.available}</div>
            </div>
            <div className="card" style={{ padding: '1rem' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Sold</div>
              <div style={{ fontSize: '1.8rem', fontWeight: 'bold', marginTop: '0.5rem', color: '#ef4444' }}>{stats.sold}</div>
            </div>
            <div className="card" style={{ padding: '1rem' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Total Net Weight</div>
              <div style={{ fontSize: '1.8rem', fontWeight: 'bold', marginTop: '0.5rem' }}>{stats.totalNetWeight.toFixed(2)}g</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <button 
            onClick={() => { setEditingItem(null); setShowForm(!showForm); setCategorySearch(''); setFormData({ name: '', metal: 'gold', purity: '22K', grossWeight: '', lessWeight: '0', categoryId: '' }); }}
