...
export default function Karigar() {
  ...
  useEffect(() => {
    const namesSet = new Set(
      transactions
        .map(t => t.itemName.toLowerCase().trim())
    );
  ...
  const handleCreateNewKarigar = () => {
    const trimmedName = searchInput.toLowerCase().trim();
    if (!trimmedName) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await karigarAPI.create({
        date: formData.date,
        type: formData.type,
        itemName: formData.itemName.toLowerCase().trim(),
  ...
  // Get transactions for selected Karigar
  const getKarigarTransactions = (name) => {
    return transactions.filter(t => t.itemName.toLowerCase().trim() === name.toLowerCase().trim());
  };
          ...
          <form onSubmit={handleSubmit} style={{ marginBottom: '32px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  type="text"
                  value={formData.itemName || searchInput}
                  onChange={handleSearchInput}
                  onFocus={() => setShowDropdown(true)}
                  placeholder={formData.type === 'given' ? 'Enter new or existing name' : 'Search or select Karigar'}
                  className="input"
                  style={{ width: '100%' }}
                  autoComplete="off"
                  required
        ...
        <div style={{
          ...
          <div className="card" style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ marginTop: '4px', fontSize: '1rem', textTransform: 'capitalize' }}>{selectedTransaction.itemName}</div>
            </div>
